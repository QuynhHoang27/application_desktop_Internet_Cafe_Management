...
class quanlyphong_thuong(QMainWindow):
    def __init__(self,ten_tai_khoan):
        super(quanlyphong_thuong, self).__init__()
        uic.loadUi('/Users/hoangquynh/BTL_DOAN_CNPM/interface/interface_qlphong3.ui', self)
        self.btn_them.clicked.connect(self.them_phong)
        self.tb_phong_thuong.setColumnCount(4)
        self.tb_phong_thuong.setHorizontalHeaderLabels(["Mã", "Tên", "Loại phòng","Giá phòng"])
        self.tb_phong_thuong.setColumnWidth(0,80)
        self.tb_phong_thuong.setColumnWidth(1,210)
        self.tb_phong_thuong.setColumnWidth(2,210)
        self.tb_phong_thuong.setColumnWidth(3,210)
 
        self.tb_phong_thuong.cellClicked.connect(self.load_data_from_table)
    ...
    def loaddata(self):
        try:
            ...
            self.cursor.execute(query, ("THƯỜNG",))
            # self.cursor.execute(query)
            data = self.cursor.fetchall()
            # Hiển thị dữ liệu lên table
            self.tb_phong_thuong.setRowCount(len(data))
            self.tb_phong_thuong.setColumnCount(len(data[0]) if data else 4)
            
            for row_idx, row_data in enumerate(data):
                for col_idx, cell_data in enumerate(row_data):
                    self.tb_phong_thuong.setItem(row_idx, col_idx, QTableWidgetItem(str(cell_data)))
    ...
    #     query = "SELECT MAX(ma_phong) FROM quan_ly_phong"
    #     self.cursor.execute(query)
    #     result = self.cursor.fetchone()

    #     if result and result[0]:
    #         chuoi_ma = result[0]
    #         so = int(chuoi_ma.replace("MPT", ""))  
    #         ma_moi = f"MPT{so+1:03d}"           
    #         return ma_moi
    #     else:
    #         return "MPT001"  

    def tao_tu_dong_maphong(self):
        query = "SELECT MAX(CAST(SUBSTRING(ma_phong, 4) AS UNSIGNED)) FROM quan_ly_phong WHERE ma_phong LIKE 'MPT%'"
        self.cursor.execute(query)
        result = self.cursor.fetchone()

        if result and result[0]:
            so = int(result[0])
            return f"MPT{so+1:03d}"
        else:
            return "MPT001"
    ...
    def them_phong(self):
        ...
        try:
            ...
            VALUES (%s, %s, %s, %s)
            """
            values = (ma_phong, ten_phong, "THƯỜNG", don_gia)
    ...
    def update(self):
        ...
        try: 
            ...
            SET ten_phong = %s, don_gia = %s  WHERE ma_phong = %s
            """
    ...
    def search(self):
        ...
        try:
            ...
            WHERE loai_phong = 'THƯỜNG'
            AND (ten_phong LIKE %s OR don_gia LIKE %s )
            """
            values = tuple(f"%{keyword}%" for _ in range(2))
            self.cursor.execute(query, values)
            data = self.cursor.fetchall()
            self.tb_phong_thuong.setRowCount(0)

            # Hiển thị kết quả tìm kiếm lên bảng
            for row_idx, row_data in enumerate(data):
                self.tb_phong_thuong.insertRow(row_idx)
                for col_idx, cell_data in enumerate(row_data):
                    self.tb_phong_thuong.setItem(row_idx, col_idx, QTableWidgetItem(str(cell_data)))
            self.clear_input()
        except mysql.connector.Error as e:
    ...
    def load_data_from_table(self):
        click = self.tb_phong_thuong.currentRow()
        if click < 0:
            return  # Không có hàng nào được chọn
        ma_phong = self.tb_phong_thuong.item(click,0).text()
        ten_phong = self.tb_phong_thuong.item(click, 1).text()
        don_gia = self.tb_phong_thuong.item(click, 3).text()
...
# quanlyphong_thuong_form = quanlyphong_thuong()
# widget.addWidget(quanlyphong_thuong_form)
# widget.setCurrentWidget(quanlyphong_thuong_form)
