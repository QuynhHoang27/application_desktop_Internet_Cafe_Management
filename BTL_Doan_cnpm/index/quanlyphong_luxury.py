...
class quanlyphong_luxury(QMainWindow):
    def __init__(self,ten_tai_khoan):
        super(quanlyphong_luxury, self).__init__()
        uic.loadUi('/Users/hoangquynh/BTL_DOAN_CNPM/interface/interface_qlphong_1.ui', self)
        self.btn_them.clicked.connect(self.them_phong)
        self.tb_phong_luxury.setColumnCount(4)
        self.tb_phong_luxury.setHorizontalHeaderLabels(["Mã", "Tên", "Loại phòng","Giá phòng"])
        self.tb_phong_luxury.setColumnWidth(0,80)
        self.tb_phong_luxury.setColumnWidth(1,210)
        self.tb_phong_luxury.setColumnWidth(2,210)
        self.tb_phong_luxury.setColumnWidth(3,210)

        self.tb_phong_luxury.cellClicked.connect(self.load_data_from_table)
    ...
    def loaddata(self):
        try:
            ...
            self.cursor.execute(query, ("LUXURY",))
            # self.cursor.execute(query)
            data = self.cursor.fetchall()
            # Hiển thị dữ liệu lên table
            self.tb_phong_luxury.setRowCount(len(data))
            self.tb_phong_luxury.setColumnCount(len(data[0]) if data else 4)
            
            for row_idx, row_data in enumerate(data):
                for col_idx, cell_data in enumerate(row_data):
                    self.tb_phong_luxury.setItem(row_idx, col_idx, QTableWidgetItem(str(cell_data)))
        
            self.clear_input()
        except mysql.connector.Error as e:
            print(f"Lỗi MySQL: {e}")
    def tao_tu_dong_maphong(self):
        query = "SELECT MAX(CAST(SUBSTRING(ma_phong, 4) AS UNSIGNED)) FROM quan_ly_phong WHERE ma_phong LIKE 'MPL%'"
        self.cursor.execute(query)
        result = self.cursor.fetchone()

        if result and result[0]:
            so = int(result[0])
            return f"MPL{so+1:03d}"
        else:
            return "MPL001"
    ...
    def them_phong(self):
        ...
        try:
            ...
            VALUES (%s, %s, %s, %s)
            """
            values = (ma_phong, ten_phong, "LUXURY", don_gia)
    ...
    def search(self):
        ...
        try:
            ...
            WHERE loai_phong = 'LUXURY' 
            AND (ten_phong LIKE %s OR don_gia LIKE %s  )
            """
            values = tuple(f"%{keyword}%" for _ in range(2))
            self.cursor.execute(query, values)
            data = self.cursor.fetchall()
            self.tb_phong_luxury.setRowCount(0)

            # Hiển thị kết quả tìm kiếm lên bảng
            for row_idx, row_data in enumerate(data):
                self.tb_phong_luxury.insertRow(row_idx)
                for col_idx, cell_data in enumerate(row_data):
                    self.tb_phong_luxury.setItem(row_idx, col_idx, QTableWidgetItem(str(cell_data)))
    ...
    def load_data_from_table(self):
        click = self.tb_phong_luxury.currentRow()
        if click < 0:
            return  
        ten_phong = self.tb_phong_luxury.item(click, 1).text()
        don_gia = self.tb_phong_luxury.item(click, 3).text()
...
# quanlyphong_luxury_form = quanlyphong_luxury()
# widget.addWidget(quanlyphong_luxury_form)
# widget.setCurrentWidget(quanlyphong_luxury_form)
